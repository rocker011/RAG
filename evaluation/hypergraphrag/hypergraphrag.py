...
class HyperGraphRAG:
    ...
    embedding_func: EmbeddingFunc = field(default_factory=default_embedding_func)
    ...
    entity_summary_llm_max_async: int = 1
    ...
    def __post_init__(self):
        ...
        embedding_impl = getattr(getattr(self.embedding_func, "func", None), "__name__", "")
        if embedding_impl == "local_qwen3_embedding":
            if self.embedding_batch_num > 8:
                logger.info(
                    "Local embedding detected; reducing embedding_batch_num from %s to 8",
                    self.embedding_batch_num,
                )
                self.embedding_batch_num = 8
            if self.embedding_func_max_async > 1:
                logger.info(
                    "Local embedding detected; reducing embedding_func_max_async from %s to 1",
                    self.embedding_func_max_async,
                )
                self.embedding_func_max_async = 1
