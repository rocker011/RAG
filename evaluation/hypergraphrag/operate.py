...
async def _handle_entity_relation_summary(
    ...
    summary_llm_max_async = max(
        1, int(global_config.get("entity_summary_llm_max_async", 1))
    )
    ...
    summary_llm_func = _SUMMARY_LLM_FUNC_CACHE.get(
        (id(use_llm_func), summary_llm_max_async)
    )
    if summary_llm_func is None:
        summary_llm_func = limit_async_func_call(summary_llm_max_async)(use_llm_func)
        _SUMMARY_LLM_FUNC_CACHE[(id(use_llm_func), summary_llm_max_async)] = (
            summary_llm_func
        )
    summary = await summary_llm_func(use_prompt, max_tokens=summary_max_tokens)
    return summary
...
async def kg_query(
    ...
    # Handle parsing error
        ...
        logger.warning("JSON parsing error: %s %s", e, final_result)
