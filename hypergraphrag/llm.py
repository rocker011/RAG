...
LOCAL_EMBED_MAX_LENGTH = 8192


def default_embedding_func():
...
@lru_cache(maxsize=2)
def _load_local_qwen3_embedding_model(model_name: str):
    if not torch.cuda.is_available():
        logger.warning(
            "Local embedding model %s is running on CPU because CUDA is unavailable",
            model_name,
        )
    if not getattr(torch.backends, "cuda", None) and torch.cuda.is_available():
        logger.warning("CUDA is available but the current PyTorch build does not expose CUDA backends")
    ...
    model.eval()
    logger.info("Loaded local embedding model %s on %s", model_name, device)
    return tokenizer, model


def _local_qwen3_embedding_sync(texts: list[str], model_name: str) -> np.ndarray:
    tokenizer, model = _load_local_qwen3_embedding_model(model_name)
...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    resolved_api_key = ensure_openai_api_key(api_key)
    resolved_base_url = get_openai_base_url(base_url)
...
async def openai_complete(
    ...
    model_name = get_openai_model(kwargs["hashing_kv"].global_config["llm_model_name"])
...
@wrap_embedding_func_with_attrs(
    embedding_dim=1024, max_token_size=8192, concurrent_limit=1
)
async def local_qwen3_embedding(
    texts: list[str], model: Optional[str] = None, **kwargs
) -> np.ndarray:
    _, resolved_model = parse_embed_model_spec(model=model)
    return await asyncio.to_thread(_local_qwen3_embedding_sync, texts, resolved_model)
...
async def openai_embedding(
    ...
    response = await openai_async_client.embeddings.create(
        model=resolved_model, input=texts, encoding_format="float"
    )
